CREATE
OR REPLACE VIEW `transfer_flights` AS (
  select
    CONCAT(`a1`.name, ', ', `a1`.country) as `start airport`,
    `f1`.flight_number as `first flight number`,
  ...
  from
    ...
    join `airport` `a1` on `f1`.origin_id = `a1`.id
    join `airport` `a2` on `f1`.destination_id = `a2`.id
    join `airport` `a3` on `f2`.destination_id = `a3`.id
  where
    datediff(`f2`.departure_date, `f1`.arrival_date) <= 1
